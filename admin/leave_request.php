...
// Construct the query
$query = "SELECT l.id, l.leave_type_id, l.requested_days, l.from_date, l.to_date, l.remarks, l.created_date, l.reviewed_by, l.reviewed_date, l.leave_status, l.empid, e.first_name, e.middle_name, e.last_name 
          FROM tblleave l
          JOIN tblemployees e ON l.empid = e.emp_id
          JOIN 
            tblleavetype lt ON l.leave_type_id = lt.id";

// Check if a specific leave status is selected
if ($leaveStatusFilter !== 'Show all') {
    $query .= " WHERE l.leave_status = ?";
...
if ($leaveStatusFilter !== 'Show all') {
    // Bind the leave status filter parameter
